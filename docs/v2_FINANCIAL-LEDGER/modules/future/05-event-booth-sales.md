...
서울코믹월드(서코, 부코, 수코 등)와 일러스타 페스 등 오프라인 행사에서 개인 창작자 또는 소규모 팀이 부스를 운영할 때 필요한 재고, 현장 판매, 예약 수령, 행사 후 정산을 관리하는 기능입니다.

이 기능은 선입금 폼, 통판 폼, 주문 접수 서비스를 대체하지 않습니다. 윗치폼처럼 이미 강한 외부 서비스를 주문 원본으로 두고, 행사 현장에서 필요한 **운영판** 역할을 맡습니다.

Fieldstack 안에서 모듈로 제공할 수도 있지만, 이 기능만 필요한 사용자가 Fieldstack 전체를 설치해야 하는 부담을 줄이기 위해 **별도 서버/단독 앱 형태**로도 제공할 수 있는 구조를 검토합니다.
...
이 기능은 Fieldstack 내부 모듈로만 제한하지 않고, 다음 두 가지 형태를 함께 검토합니다.

### 1. Fieldstack 통합 모드

Fieldstack을 이미 사용하는 사용자를 위한 형태입니다.

장점:
- Ledger 모듈과 행사 매출·비용을 바로 연결할 수 있습니다.
- 세무 준비 지원 기능으로 행사 정산 자료를 넘길 수 있습니다.
- 사용자, 파일, 권한, 백업, 설정 등 Fieldstack 공통 기능을 재사용할 수 있습니다.

적합한 사용자:
- Fieldstack으로 개인 금융과 자료 정리를 함께 관리하려는 사용자
- 행사 매출과 비용을 장기적으로 추적하려는 사용자
- 세무사에게 전달할 참고자료까지 함께 정리하려는 사용자

### 2. 단독 서버/앱 모드

부스 운영 기능만 필요한 사용자를 위한 형태입니다.

장점:
- Fieldstack 전체 설치 없이 행사 운영 도구만 사용할 수 있습니다.
- 개인 창작자 또는 소규모 팀이 행사 전용 서버로 가볍게 올려 사용할 수 있습니다.
- Fieldstack의 금융·세무 기능을 쓰지 않는 사용자에게 진입 장벽이 낮습니다.

제약:
- Ledger 자동 연동은 기본 제공되지 않습니다.
- 세무 준비 지원 기능과의 연결은 내보내기 파일 또는 별도 연동 옵션으로 처리해야 합니다.
- 사용자·권한·백업·파일 보관 정책을 단독 앱 안에서 별도로 제공해야 합니다.

### 3. 공통 코어 분리 방향

가능하면 핵심 로직을 Fieldstack 전용 구현에 묶지 않습니다.
...
Fieldstack 모드는 이 공통 코어를 Fieldstack 모듈 API, Ledger, 세무 준비 지원 기능과 연결합니다. 단독 모드는 같은 공통 코어를 자체 서버, 자체 DB, 자체 UI에 연결합니다.

이 구조는 Fieldstack 사용성을 일부 분산시킬 수 있지만, 부스 운영 기능만 필요한 사용자의 설치 부담을 줄이고 기능 자체의 활용 범위를 넓히는 장점이 있습니다.
...
이 기능은 결제 자체를 처리하지 않습니다. 카드 단말기, 계좌이체, 간편결제 등 외부에서 결제가 끝난 결과를 운영자가 기록하는 방식입니다.
...
이 기능은 윗치폼의 폼 운영 기능을 대체하지 않고, 윗치폼에서 내려받은 엑셀 데이터를 행사 운영용 참고 데이터로 가져와 재고, 수령, 현장 판매, 행사 후 정산을 돕는 방향으로 검토합니다.
