...
기존의 "Fieldstack 앱이 AI API를 호출하는 방식(API Key)" 외에, **사용자가 이미 구독 중인 외부 AI(ChatGPT Plus, Gemini Advanced) 화면에서 Fieldstack 데이터를 직접 조회할 수 있도록 연결하는 역방향 연동 방식**입니다.

### 기대 효과 (이중 과금 방지 및 고급 활용)
- **비용 절감:** 사용자가 이미 결제하고 있는 AI 구독(B2C) 모델을 활용하여 API 요금(B2B)의 이중 지출을 방지합니다.
- **고급 컨설팅:** "내 데이터 분석해서 파이썬으로 차트 그려줘", "이번 달 예산 시뮬레이션 해줘" 등 외부 거대 AI의 풀 파워를 채팅창에서 그대로 활용할 수 있습니다.
- 앱 내부(프론트엔드)에서는 가벼운 무료 AI(Gemini Free, Ollama)로 영수증 자동화만 처리하고, 심도 깊은 분석은 사용자의 주력 AI 챗봇에게 맡기는 완벽한 투트랙(Two-track) 구성이 가능해집니다.

### 기술적 구현 방향 (검토 중)
1. **OpenAPI 명세 제공:** Fieldstack 서버가 외부 AI가 읽을 수 있는 `openapi.json` 스펙을 제공합니다.
2. **읽기 전용(Read-only) API Key:** 외부 AI가 실수로 데이터를 삭제하거나 수정하지 못하도록, 철저히 '조회' 권한만 가진 전용 토큰을 발급하는 기능을 제공합니다.
3. **외부망 터널링 지원:** ChatGPT 서버가 개인의 Fieldstack 서버에 접근할 수 있도록, Cloudflare Tunnel이나 Ngrok 같은 무료 터널링 서비스의 쉬운 연동 가이드 또는 내장 기능을 제공합니다.

### 미확정 사유
Fieldstack의 핵심 철학인 **프라이버시(폐쇄망 유지)**와 상충되는 부분이 있습니다. 외부 AI가 내 가계부 DB를 읽어가도록 허용하는 것은 결국 개인 금융 데이터를 빅테크 서버로 넘기는 것과 같으므로, 보안 책임을 오롯이 사용자가 져야 하는 파워 유저(Power User) 전용 플러그인(Add-on) 형태로만 고려되고 있습니다.