...
## 11. File Upload Handling Policy

Fieldstack Core는 파일 업로드를 **인식하고 전달하는 역할만** 수행한다.

- Core는 파일의 의미를 해석하지 않는다.
- Core는 파일 내용을 직접 파싱하지 않는다.
- Core는 파일 처리 결과에 대한 책임을 가지지 않는다.

파일의 실제 처리 방식(검증, 파싱, 저장, 반영)은  
항상 이를 선언한 Module의 책임이다.
