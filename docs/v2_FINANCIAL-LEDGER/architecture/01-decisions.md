...
## 결정 #5: Hard-Core 계층 폐기 및 Core 단일 통합

### ✅ 결정 사항

Hard-Core를 별도 계층으로 분리하지 않고 **Core 단일 프로세스로 통합**합니다.

### 📖 배경

초기 설계 당시 Hard-Core는 Valorant Vanguard처럼 Core와 프로세스 수준에서 분리된 독립 감시 계층으로 계획되었습니다. 이후 검토 과정에서 다음이 확인되었습니다:

- Ring 0(커널 레벨) 동작은 OS 커널 드라이버 작성이 필요하며, 일반 애플리케이션 프레임워크 내에서는 불가능
- Ring 3(사용자 공간) 기준으로도 Hard-Core를 별도 프로세스로 분리할 수 있으나, 이 경우 Core와 동일한 권한 레벨에서 동작

### 🔍 고려했던 옵션

#### Option A: Hard-Core 별도 프로세스 유지 (Ring 3)
```
장점:
- 개념적 분리 유지

단점:
- Core와 동일한 Ring 3에서 동작 → 분리의 실익 없음
- 두 프로세스 간 IPC/소켓 통신 유지 비용
- 셀프호스팅 사용자가 두 프로세스를 관리해야 함
- DB 꼬임 등 운영 이슈는 어차피 Core 내부에서 처리해야 함
```

#### Option B: Core 단일 통합 + 외부 프로세스 매니저 ⭐ (선택)
```
장점:
- 구현/유지보수 단순
- 운영 이슈(DB 재연결, 꼬인 트랜잭션)를 Core 내부에서 직접 처리
- 크래시 복구는 Docker/PM2/systemd에 위임
- Go 전환 계획 불필요 (Core는 Node.js/TypeScript 유지)

단점:
- Core 크래시 시 감시 주체가 없음 (외부 프로세스 매니저로 대체)
```
...
1. **분리의 실익 없음** — Ring 3에서 동작하는 이상 Hard-Core는 Core와 동등한 신뢰 레벨이며, 독립 감시자로서의 의미가 없음
2. **셀프호스팅 특성** — 개인/소수 사용자 대상 앱에서 고가용성 이중화 구조는 과도한 복잡도
3. **충분한 대체 수단** — DB 연결 재시도 + 트랜잭션 타임아웃 + Docker restart 정책으로 실질적 안정성 확보 가능

### 🎯 안정성 확보 방향

| 이슈 유형 | 처리 방법 |
|---|---|
| DB 연결 끊김 | 지수 백오프 기반 자동 재연결 |
| 쿼리 타임아웃 / 데드락 | DB `statement_timeout` + 에러 캐치 후 재시도 |
| 유휴 트랜잭션 고착 | PostgreSQL `idle_in_transaction_session_timeout` |
| 프로세스 크래시 | Docker `restart: unless-stopped` / PM2 / systemd |

### 📚 관련 문서

> 📖 **기술 스택:**
> → `technical/00-tech-stack.md § 안정성 & 복구`
...
구현체: `packages/core/src/utils/validation.ts`의 `validatePassword()` 함수.
...
## 결정 #7: 이메일 전송 방식 — SMTP 설정 기반

### ✅ 결정 사항

**사용자가 직접 외부 SMTP 서버 정보를 설정**하여 이메일을 전송하는 방식을 채택합니다.  
Fieldstack이 자체 메일 서버를 운영하지 않으며, 전용 이메일 서비스(SendGrid 등)도 강제하지 않습니다.

### 📖 배경

비밀번호 복구 링크 전송, 구독 알림 등 이메일 발송이 필요한 기능이 존재합니다.  
Fieldstack은 self-hosted 앱이므로 사용자가 선호하는 외부 SMTP 서버를 자유롭게 연결할 수 있어야 합니다.

### 🔍 고려했던 옵션

#### Option A: 전용 이메일 서비스 고정 (SendGrid, Resend 등)
```
장점:
- 높은 전송 신뢰도
- 별도 서버 불필요
...
#### Option B: Self-hosted SMTP 서버 (Postfix 등)
```
장점:
- 완전한 자체 운영
...
#### Option C: 외부 SMTP 서버 연동 (사용자 설정) ⭐ (선택)
```
장점:
- 사용자가 기존에 쓰는 이메일 서비스 그대로 활용 가능
- Gmail, Outlook, Naver 등 어디든 연결 가능
- Fieldstack이 특정 서비스에 종속되지 않음
- 설치 초기 SMTP가 없어도 앱 사용 가능 (Lazy 이메일 정책과 일관)

단점:
- 사용자가 직접 SMTP 정보를 입력해야 함
```
...
1. **self-hosted 철학과 일치** — 특정 외부 서비스에 묶이지 않음
2. **Lazy 이메일 검증 정책(결정 #6)과 일관성** — SMTP 없이도 앱이 동작하며, 비밀번호 복구가 필요한 시점에만 SMTP 설정이 요구됨
3. **유연성** — Gmail, Outlook, Naver, 기업 SMTP 등 어떤 서버든 호스트/포트/인증 정보만 있으면 연결 가능
...
| 서비스 | 호스트 | 포트 | 비고 |
|---|---|---|---|
| Gmail | smtp.gmail.com | 587 | App Password 필요 (2FA 활성화 시) |
| Outlook / Hotmail | smtp-mail.outlook.com | 587 | 계정 비밀번호 사용 |
| Naver | smtp.naver.com | 587 | SMTP 사용 설정 필요 |
| 기타 | 사용자 입력 | 사용자 입력 | 임의 SMTP 서버 |
...
SMTP 포트      예: 587
보안           TLS / STARTTLS / None
사용자 이름    예: yourname@gmail.com
비밀번호       App Password 또는 계정 비밀번호
발신자 이름    예: Fieldstack
발신자 이메일  예: yourname@gmail.com
```

#### 이메일 발송 대상 기능 (Phase 2~)

- 비밀번호 복구 링크 발송
- 구독 결제일 알림 (D-7, D-3, D-1)
- 신규 유저 임시 비밀번호 발송 (선택 — SMTP 설정 완료 시)

#### SMTP 미설정 시 동작

- 비밀번호 복구: SMTP 미설정 안내 + 관리자 임시 비밀번호 발급 경로 안내
- 구독 알림: 알림 기능 비활성화 상태 유지
...
1. **SMTP 비밀번호 암호화 저장** — DB에 평문 저장 금지, Argon2id 또는 AES-256 암호화
2. **연결 테스트 제공** — Admin 설정 화면에서 테스트 이메일 발송 기능 제공
3. **Gmail 사용 시 App Password 안내** — Google 계정 2FA 활성화 상태에서는 계정 비밀번호 직접 사용 불가, App Password 발급 필요
...
| #7 이메일 전송 | 외부 SMTP 연동 | self-hosted 철학, 서비스 무종속 | ✅ 확정 |
...
| 2026-04-12 | 1.4.0 | 이메일 전송 방식 결정 추가 — 외부 SMTP 연동, 지원 서버 목록, 미설정 시 동작 정의 (#7) |
